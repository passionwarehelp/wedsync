    ...
    <View className="flex-1 bg-neutral-50">
      ...
      >
        <View className="flex-row items-center justify-between mb-6">
          <View>
            <Text className="text-white text-3xl font-bold">WedSync</Text>
            <Text className="text-white/90 text-base mt-1">Professional Dashboard</Text>
          </View>
          <Pressable
            onPress={() => navigation.navigate("CreateWedding")}
            className="bg-white rounded-full w-12 h-12 items-center justify-center shadow-lg"
          >
            <Ionicons name="add" size={28} color="#C9A961" />
          </Pressable>
        </View>

        <View className="bg-white/95 rounded-2xl flex-row items-center px-4 py-3">
          ...
          <View className="space-y-4 pb-8">
            {filteredWeddings.map((wedding) => (
              <Pressable
                ...
                className="bg-white rounded-2xl overflow-hidden shadow-sm active:opacity-70"
              >
                <View className="p-5">
                  <View className="flex-row items-start justify-between mb-3">
                    <View className="flex-1">
                      <Text className="text-neutral-800 text-xl font-semibold">{wedding.coupleName}</Text>
                    ...
                    <View
                      className={`px-3 py-1 rounded-full ${
                        wedding.status === "upcoming" ? "bg-emerald-100" : "bg-blue-100"
                      }`}
                    >
                      <Text
                        className={`text-xs font-medium ${
                          wedding.status === "upcoming" ? "text-emerald-700" : "text-blue-700"
                        }`}
                      >
                        {wedding.status}
                      </Text>
                  ...
                  <View className="flex-row items-center mb-4">
                    ...
                    {wedding.venue && (
                      <>
                        ...
                        <Text className="text-neutral-600 text-sm ml-1 flex-1" numberOfLines={1}>
                          {wedding.venue}
                        </Text>
                      </>
                    )}
                ...
                <View className="h-1 bg-gradient-to-r from-amber-400 to-amber-200" />
              </Pressable>
            ))}
          </View>
        )}
